...
def evaluate(model, test_dset, dataloader, data_root, cache):
    ...
    for _, (v, b, q, a) in enumerate(iter(dataloader)):
        ...
        for i0 in range(len(ans)):
            i = _ * 50 + i0
            # res = {"image":img[i],"ques":qt[i],"true":ans[i],"pred":ans2[i]}
            # print("image:",img[i],"ques:",qt[i],"true:",ans[i],"pred:",ans2[i],ans2[i]==ans[i])
            if ans[i0]!= ans2[i0]:
...
if __name__ == '__main__':
    ...
    # cache = "cache_0807"
    dictionary = Dictionary.load_from_file('{}/dictionary_chest.pkl'.format(cache))#  dictionary1.pkl
    # train_dset = VQAFeatureDataset('train', dictionary)
    # eval_dset = VQAFeatureDataset('val', dictionary)
    # train_answer_file = './{}/{}/train_annotations.json'.format(data_root,cache)
    # val_answer_file = './{}/{}/val_annotations.json'.format(data_root,cache)
    ...
    model = getattr(base_model, constructor)(train_dset, args.num_hid, emb_dim, answer_np, mask).cuda()
    ...
    train(model, train_loader, eval_loader, cache, args.epochs, output_file)
