...
def __load_dataset(dataroot, name, img_id2val):
    """Load entries
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data'):
        ...
        with h5py.File(h5_path, 'r') as hf:
            self.features = np.array(hf.get('image_features')).astype("float32")
            self.spatials = np.array(hf.get('spatial_features')).astype("float32")
