...
def create_dictionary_chest(dataroot):
    ...
    for path in anno_files:
        anno_path = os.path.join(dataroot, path)
        qs = json.load(open(anno_path))['annotations']
        for q in qs:
            dictionary.tokenize(q['answers'][0]['answer'], True)
    return dictionary
...
def parse_args():
    ...
    parser.add_argument('--cache', type=str, default="1014")
...
if __name__ == '__main__':
    ...
    weights_new = np.array([model[w] for w in d.idx2word])
    np.save('%s/gensim_init_%dd_chest.npy' % (cache, emb_dim), weights_new)
