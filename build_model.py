...
class MultiLabelModel(nn.Module):
    def __init__(self, basemodel, basemodel_output, num_classes):
        ...
        # self.fc = {}
        # for index, num_class in enumerate(num_classes):
        #     self.fc[index] = nn.Linear(basemodel_output, num_class)
        self.fc = []
        for num_class in num_classes:
            self.fc.append(nn.Linear(basemodel_output, num_class))
...
class ClassificationModel(nn.Module):
    def __init__(self, basemodel, basemodel_output, num_class):
        super(ClassificationModel, self).__init__()
        self.basemodel = basemodel
        self.num_class = num_class
        # self.fc1 = nn.Linear(basemodel_output, 200)
        # self.bn = nn.BatchNorm1d(num_features=200, momentum=0.1)
        # self.fc2 = nn.Linear(200, num_class)
        
        self.fc = nn.Linear(basemodel_output, num_class)
    
    def forward(self, x):
        x = self.basemodel.forward(x)
        out = self.fc(x)
        # out1 = self.fc1(x)
        # out = self.fc2(self.bn(out1))
        return out

def BuildClassificationModel(basemodel, basemodel_output, num_class):
    return ClassificationModel(basemodel, basemodel_output, num_class)