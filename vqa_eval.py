...
class VQAFeatureDataset(Dataset):
    def __init__(self, questions, dictionary, train_target, dataroot='data', cache='data/cache'):
...
# file_name = "1116_unique"
# root_path = "/home/ymeng/8888/embedding_question/data_saved/%s"%file_name 
# # question_answers_pool_dict = json.load(open("./data_ori/question2answers_dict.json","r"))
# question_answers_pool_dict = json.load(open("%s/question2answer_dict_sent_unique.json"%root_path,"r"))
# questions = list(question_answers_pool_dict.keys())
def generate_ques_ans_list(model, train_dset, cache, questions):
    batch_size = 64
    ...
    # ans2label = pickle.load(open(os.path.join(cache, 'trainval_ans2label.pkl'), 'rb'))
    label2ans = pickle.load(open(os.path.join(cache, 'trainval_label2ans.pkl'), 'rb'))
    generate_qa_pair_dict = {}
    for qi, ai, q in zip(questions_list, answers_list, questions_json):
        # ques = vqa.questions[qi]
        ques = questions[qi]
        ans = label2ans[ai]
        image_id = q['image_id']
        if image_id not in generate_qa_pair_dict:
            generate_qa_pair_dict[image_id] = []
        generate_qa_pair_dict[image_id].append([ques, ans])
...
def parse_args():
    ...
    parser.add_argument('--file_tag', type=str, default="1116")
...
if __name__ == "__main__":
    ...
    train_target = "{}/test_target.pkl".format(cache)  #didn't use this in test part   
    questions_json = json.load(open("%s/questions.json"%cache,"r"))
    train_dset = VQAFeatureDataset(questions_json, dictionary, train_target, dataroot=dataroot, cache=cache)

    saved_path = "/home/ymeng/store_ym/medical_report_vqa/data_saved/%s/vqa_eval"%file_name
    if os.path.isdir(saved_path) == False:
        os.makedirs(saved_path)

    question_file = "%s/questions_classifier_train.json"%saved_path 
    json.dump(questions_json, open(question_file,'w'))
    model = torch.load("./saved_lungs/cache_%s/model.pth"%(file_name))  
    
    
    questions = json.load(open("/home/ymeng/store_ym/medical_report_vqa/data/X_ray/cache0/word_unique.json"))
    generate_qa_pair_dict = generate_ques_ans_list(model, train_dset, cache, questions)
